...
export class DomainToken extends ethereum.SmartContract {
  static bind(address: Address): DomainToken {
    return new DomainToken("DomainToken", address);
  }
  ...
  mint(account: Address, amount: BigInt, data: Bytes): boolean {
    let result = super.call("mint", "mint(address,uint256,bytes):(bool)", [
  ...
  try_mint(
    ...
    let result = super.tryCall("mint", "mint(address,uint256,bytes):(bool)", [
      ethereum.Value.fromAddress(account),
      ethereum.Value.fromUnsignedBigInt(amount),
      ethereum.Value.fromBytes(data)
    ]);
